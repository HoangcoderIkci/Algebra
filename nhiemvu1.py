...
def khoi_tao_pole(p=None):
    global Mang_phep_cong, Mang_phep_nhan, Mang_doi_cong, Mang_nghich_dao, Mang_phep_tru, Mang_phep_chia, Character_Pole
    if p is None:
        Character_Pole = int(input("Character pole: "))
    return init_all_arrays()
...
def poly_divide(poly1, poly2):
    ...
    while vi_tri_khac_0 <= do_lech:
        ...
        vi_tri_khac_0 = tim_vi_tri_dau_tien_khac_0(p1_cp)
    k = 0
    while k < len(p1_cp) and p1_cp[k] == 0:
        k += 1
...
def gcd_poly(poly1, poly2):
    ...
    while True:
        ...
        if np.all(temp == 0):
            hs_nghich = Mang_nghich_dao[p2_cp[0]]
            res = [Mang_phep_nhan[hs_nghich][i] for i in p2_cp]
            return np.array(res)
        p1_cp = p2_cp
        p2_cp = temp
